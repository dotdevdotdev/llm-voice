...
# ElevenLabs API URL
ELEVENLABS_API_URL = f"https://api.elevenlabs.io/v1/text-to-speech/{config.elevenlabs_voice_id}"
...
async def text_to_speech(text: str, session: aiohttp.ClientSession) -> Optional[str]:
    ...
    try:
        async with session.post(ELEVENLABS_API_URL, json=data, headers=headers) as response:
            if response.status == 200:
                ...
                logger.error(f"Error in text-to-speech request: {await response.text()}")
                return None
    except Exception as e:
